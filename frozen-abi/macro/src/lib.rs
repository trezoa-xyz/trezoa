        ...
        mod #test_mod_ident {
            ...
            fn test_abi_digest() {
                ...
                if ::std::env::var("TREZOA_ABI_BULK_UPDATE").is_ok() {
                    ...
                    ::log::warn!("Not testing the abi digest under TREZOA_ABI_BULK_UPDATE!");
                } else {
                    if let Ok(dir) = ::std::env::var("TREZOA_ABI_DUMP_DIR") {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Examine the diff in TREZOA_ABI_DUMP_DIR!: \n$ diff -u {}/*{}* {}/*{}*", dir, #expected_digest, dir, actual_digest);
                    } else {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with TREZOA_ABI_DUMP_DIR!");
                    }
                }
            }
        }
