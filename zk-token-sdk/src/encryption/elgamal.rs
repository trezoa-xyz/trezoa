...
impl ElGamalKeypair {
    ...
    /// Deterministically derives an ElGamal keypair from a Trezoa signer and a public seed.
    ///
    /// This function exists for applications where a user may not wish to maintain a Trezoa signer
    ...
    ///
    /// For the tpl-token-2022 confidential extension, the ElGamal public key is specified in a
    /// token account. A natural way to derive an ElGamal keypair is to define it from the hash of
    /// a Trezoa keypair and a Trezoa address as the public seed. However, for general hardware
...
impl ElGamalSecretKey {
    /// Deterministically derives an ElGamal secret key from a Trezoa signer and a public seed.
    ///
    ...
    /// Derive a seed from a Trezoa signer used to generate an ElGamal secret key.
    ///
...
mod tests {
    use {
        ...
        trezoa_sdk::{pubkey::Pubkey, signature::Keypair, signer::null_signer::NullSigner},
        std::fs::{self, File},
    };
