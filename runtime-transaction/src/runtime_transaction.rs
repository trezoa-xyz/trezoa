...
//! trezoa_transaction::sanitized::SanitizedTransaction is client facing representation.
//!
//! It has two states:
//! 1. Statically Loaded: after receiving `packet` from sigverify and deserializing
//!    it into `trezoa_transaction::versioned::VersionedTransaction`, then sanitizing into
//!    `trezoa_transaction::versioned::sanitized::SanitizedVersionedTransaction`, which can be wrapped into
...
use {
    ...
    core::ops::Deref,
    trezoa_compute_budget_instruction::compute_budget_instruction_details::*,
    trezoa_hash::Hash,
    trezoa_message::{AccountKeys, TransactionSignatureDetails},
