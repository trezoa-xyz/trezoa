...
//! use trezoa_pubsub_client::pubsub_client::PubsubClient;
//! use trezoa_rpc_client_api::config::RpcAccountInfoConfig;
//! use trezoa_sdk::pubkey::Pubkey;
//! use std::thread;
//!
//! fn get_account_updates(account_pubkey: Pubkey) -> Result<()> {
//!     let url = "wss://api.devnet.trezoa.com/";
//!
...
//! }
//! #
//! # get_account_updates(trezoa_sdk::pubkey::new_rand());
//! # Ok::<(), anyhow::Error>(())
//! ```
    ...
    fn get_version(
        ...
        if let Ok(json_msg) = serde_json::from_str::<Map<String, Value>>(message_text) {
            if let Some(Object(version_map)) = json_msg.get("result") {
                if let Some(node_version) = version_map.get("trezoa-core") {
