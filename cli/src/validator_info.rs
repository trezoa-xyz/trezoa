use {
    ...
    trezoa_config_program::{config_instruction, get_config_data, ConfigKeys, ConfigState},
...
fn parse_validator_info(
    ...
    if account.owner != trezoa_config_program::id() {
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&trezoa_config_program::id())?;
...
mod tests {
    ...
    fn test_parse_validator_info() {
        ...
        assert_eq!(
            parse_validator_info(
                &Pubkey::default(),
                &Account {
                    owner: trezoa_config_program::id(),
                    data,
                    ..Account::default()
                }
            )
    ...
    fn test_parse_validator_info_not_validator_info_account() {
        assert!(parse_validator_info(
            &Pubkey::default(),
            &Account {
                owner: trezoa_sdk::pubkey::new_rand(),
                ..Account::default()
            }
        )
        .unwrap_err()
