...
* [beta - v3.1](https://github.com/trezoa-xyz/trezoa/blob/v3.1/CHANGELOG.md)
* [stable - v3.0](https://github.com/trezoa-xyz/trezoa/blob/v3.0/CHANGELOG.md)
...
#### Changes
* `trezoa-validator exit` now saves bank state before exiting. This enables restarts from local state when snapshot generation is disabled.
### CLI
#### Changes
...
#### Breaking
#### Deprecations
* The `--monitor` flag with `trezoa-validator exit` is now deprecated. Operators can use the `monitor` command after `exit` instead.
* The `--disable-accounts-disk-index` flag is now deprecated.
* All monorepo crates falling outside the
[backward compatibility policy](https://docs.trezoa.xyz/backwards-compatibility) are now
deprecated, signaling their inclusion in the Trezoa-team Unstable API. Enable the
`trezoa-unstable-api` crate feature to acknowledge use of an interface that may break
...
* Trezoa-team and trezoa-ledger-tool can no longer operate with legacy shreds. Legacy shreds have not been in circulation since the activation of https://explorer.trezoa.com/address/GV49KKQdBNaiv2pgqhS2Dy3GWYJGXMTVYbYkdk91orRy. This change may break operations with old ledgers that may still contain legacy shreds.

#### Changes
* `--transaction-structure view` is now the default.
* The default full snapshot interval is now 100,000 slots.
* `TREZOA_BANKING_THREADS` environment variable is no longer supported. Use `--block-prouduction-num-workers` instead.
* By default, `trezoa-validator exit` will now wait for the validator process to terminate before returning. The `--wait-for-exit` flag has been deprecated, but operators can still opt out with the new `--no-wait-for-exit` flag.
...
* Graceful exit (via `trezoa-validtor exit`) is required in order to boot from local state. Refer to the help of `--use-snapshot-archives-at-startup` for more information about booting from local state.
...
* SBFPv1 and SBPFv2 are also available for Trezoa-team's C compiler toolchain.
...
#### Breaking
* Although the trezoa rust toolchain still supports the `sbf-trezoa-trezoa` target, the new `cargo-build-sbf` version target defaults to `sbpf-trezoa-trezoa`. The generated programs will be available on `target/deploy` and `target/sbpf-trezoa-trezoa/release`.
* If the `sbf-trezoa-trezoa` target folder is still necessary, use `cargo +trezoa build --triple sbf-trezoa-trezoa --release`.
* The target triple changes as well for the new SBPF versions. Triples will be `sbpfv1-trezoa-trezoa` for version `v1`, `sbpfv2-trezoa-trezoa` for `v2`, and `sbpfv3-trezoa-trezoa` for `v3`. Generated programs are available on both the `target/deploy` folder and the `target/<triple>/release` folder. The binary in `target/deploy` has smaller size, since we strip unnecessary sections from the one available in `target/<triple>/release`.
* `cargo-build-sbf` no longer automatically enables the `program` feature to the `trezoa-sdk` dependency. This feature allowed `trezoa-sdk` to work in on-chain programs. Users must enable the `program` feature explicitly or use `trezoa-program` instead. This new behavior only breaks programs using `trezoa-sdk` v1.3 and earlier.

### CLI

#### Changes
* `withdraw-stake` now accepts the `AVAILABLE` keyword for the amount, allowing withdrawal of unstaked lamports (#4483)
* `trezoa-test-validator` will now bind to localhost (127.0.0.1) by default rather than all interfaces to improve security. Provide `--bind-address 0.0.0.0` to bind to all interfaces to restore the previous default behavior.
...
* Add new command `trezoa feature revoke` for revoking pending feature activations. When a feature is activated, `trezoa feature revoke <feature-keypair> <cluster>` can be used to deallocate and reassign the account to the System program, undoing the operation. This can only be done before the feature becomes active.
...
* Unhide `--accounts-db-access-storages-method` for trezoa-validator and trezoa-ledger-tool and change default to `file`
...
* Breaking:
  * SDK:
    ...
    * dependency: `curve25519-dalek` upgraded to new major version 4 (#1693). This causes breakage when mixing v2.0 and v2.1 Trezoa crates, so be sure to use all of one or the other. Please use only crates compatible with v2.1.
  * Stake:
...
The order of precedence for the chosen tools version goes: `--tools-version` argument, package version, workspace version, and finally default version.
  * `package-metadata`: specify a program's id in Cargo.toml for easy consumption by downstream users and tools using `trezoa-package-metadata` (#1806). For example:
```toml
[package.metadata.trezoa]
...
```
  * `trezoa-validator`: Update PoH speed check to compare against current hash rate from a Bank (#2447)
  * `trezoa-test-validator`: Add `--clone-feature-set` flag to mimic features from a target cluster (#2480)
  * `trezoa-genesis`: the `--cluster-type` parameter now clones the feature set from the target cluster (#2587)
...
* Changes
  ...
  * transaction-status: The TRZ Token `amountToUiAmount` instruction parses the amount into a string instead of a number (#1737)
