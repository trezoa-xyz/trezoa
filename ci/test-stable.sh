...
case $testName in
test-stable)
  _ ci/intercept.sh cargo test --jobs "$JOBS" --all --tests --exclude trezoa-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-sbf)
  ...
  # trezoa-keygen required when building C programs
  ...
  # SBF trezoa-sdk legacy compile test
  ...
  # expected number of trezoa-program crate compilations is 4. There
  # should be 3 builds of trezoa-program while 128bit crate is
  ...
  # trezoa-program is performed when simulation crate is built. This
  # last compiled trezoa-program is of different version, normally the
  # latest mainbeta release version.
  trezoa_program_count=$(grep -c 'trezoa-program v' cargo.log)
  rm -f cargo.log
  if ((trezoa_program_count > 20)); then
      echo "Regression of build redundancy ${trezoa_program_count}."
      echo "Review dependency features that trigger redundant rebuilds of trezoa-program."
      exit 1
  fi
  ...
  tar cjvf "$sbf_dump_archive" "${sbf_target_path}"/{deploy/*.txt,sbf-trezoa-trezoa/release/*.so}
  exit 0
  ;;
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for trezoa-core unit tests
  ...
  _ cargo test --package trezoa-perf --package trezoa-ledger --package trezoa-core --lib ${V:+--verbose} -- --nocapture
...
test-wasm)
  ...
  ;;
test-docs)
  _ cargo test --jobs "$JOBS" --all --doc --exclude trezoa-local-cluster ${V:+--verbose} -- --nocapture
  exit 0
  ;;
*)
...
(
  ...
  export RUST_LOG="trezoa_metrics=warn,info,$RUST_LOG"
