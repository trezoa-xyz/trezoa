...
_ cargo "+${rust_nightly}" miri test -p trezoa-unified-scheduler-logic

# test big endian branch
_ cargo "+${rust_nightly}" miri test --target s390x-unknown-linux-gnu -p trezoa-vote -- "vote_state_view" --skip "arbitrary"
# test little endian branch for UB
_ cargo "+${rust_nightly}" miri test -p trezoa-vote -- "vote_state_view" --skip "arbitrary"
