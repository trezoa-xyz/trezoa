...
pub enum UpgradeableLoaderInstruction {
    ...
    /// # use trezoa_program::bpf_loader_upgradeable;
    /// # let program_address = &[];
    ...
    ///   4. `[]` Rent sysvar.
    ///   5. `[]` Clock sysvar.
    ///   6. `[]` System program (`trezoa_sdk::system_program::id()`).
    ...
    ///   2. `[]` System program (`trezoa_sdk::system_program::id()`), optional, used to transfer
