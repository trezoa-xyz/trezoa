//! Logging utilities for Rust-based Trezoa programs.
//!
...
//!
//! - The `trezoa logs` command displays logs for all transactions executed on a
...
//!   active then the `trezoa_rpc_client` crate logs at the "debug" level any logs
//!   for transactions that failed during simulation. If using [`env_logger`]
//!   these logs can be activated by setting `RUST_LOG=trezoa_rpc_client=debug`.
//! - Logs can be retrieved from a finalized transaction by calling
//!   [`RpcClient::get_transaction`].
//! - Block explorers may display logs.
//!
//! [`RpcClient`]: https://docs.rs/trezoa-rpc-client/latest/trezoa_rpc_client/rpc_client/struct.RpcClient.html
//! [`env_logger`]: https://docs.rs/env_logger
//! [`RpcClient::get_transaction`]: https://docs.rs/trezoa-rpc-client/latest/trezoa_rpc_client/rpc_client/struct.RpcClient.html#method.get_transaction
//!
...
/// for constrained environments like the Trezoa VM.
///
/// # Examples
///
/// ```
