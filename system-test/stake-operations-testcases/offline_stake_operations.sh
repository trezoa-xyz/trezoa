...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.trezoa.com:8899
  exit 1
fi
trezoa config set --url $url
...
trezoa-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
trezoa config set --keypair $dummy_keypair
...
trezoa-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
trezoa-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(trezoa-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(trezoa-keygen pubkey $online_nonce_account_keypair)"
...
trezoa-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
trezoa-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
trezoa-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
trezoa-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(trezoa-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(trezoa-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(trezoa-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(trezoa-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
trezoa airdrop 100 $offline_system_account_pubkey
trezoa airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
trezoa create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
...
trezoa-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(trezoa-keygen pubkey $stake_account_keypair)"

sign_only="$(trezoa create-stake-account $stake_account_keypair 50 \
...
trezoa create-stake-account $stake_account_keypair 50 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(trezoa show-validators)")"
nonce="$(trezoa nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(trezoa delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
trezoa delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(trezoa deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
trezoa deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
