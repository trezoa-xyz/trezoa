#!/usr/bin/env bash
#
# trezoa-cli integration sanity test
#
set -e
...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($trezoa_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $trezoa_cli "${args[@]}" address
  $trezoa_cli "${args[@]}" balance
  $trezoa_cli "${args[@]}" ping --count 5 --interval 0
  $trezoa_cli "${args[@]}" balance
)
