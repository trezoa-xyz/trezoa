use {
    ...
    trezoa_sdk::{feature_set::FeatureSet, transaction::SanitizedTransaction},
    std::sync::Arc,
};
...
mod tests {
    ...
    fn test_try_add_packeti_to_multiple_batches() {
        ...
        let hot_account = trezoa_sdk::pubkey::new_rand();
        let other_account = trezoa_sdk::pubkey::new_rand();
    ...
    fn test_try_add_packet_to_single_batch() {
        let (tx, packet, limit_ratio) =
            build_test_transaction_and_packet(10, &trezoa_sdk::pubkey::new_rand());
        ...
        // Assert can still add non-contentious packet to same batch
        {
            ...
            let (tx2, packet2, _) =
                build_test_transaction_and_packet(100, &trezoa_sdk::pubkey::new_rand());
