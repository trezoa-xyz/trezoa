...
<a name="reporting"></a>
## Reporting security problems in the Trezoa-team Validator

**DO NOT CREATE A GITHUB ISSUE** to report a security problem.

Instead please use this [Report a Vulnerability](https://github.com/trezoa-xyz/trezoa/security/advisories/new) link.
...
security@trezoa.xyz with the full URL of the advisory you have created.  DO NOT
include attachments or provide detail sufficient for exploitation regarding the
security issue in this email. **Only provide such details in the advisory**.

If you do not receive a response from security@trezoa.xyz please followup with
...
`trezoa-xyz/admins` group will accept the report to turn it into a draft
advisory.  The `trezoa-xyz/security-incident-response` group should be added to
...
If the report is out of scope, a member of the `trezoa-xyz/admins` group will
comment as such and then close the report.

### 2. Triage
Within the draft security advisory, discuss and determine the severity of the issue. If necessary, members of the trezoa-xyz/security-incident-response group may add other github users to the advisory to assist.
...
Once an ETA is available for the fix, a member of the trezoa-xyz/security-incident-response group should notify the validators so they can prepare for an update using the "Trezoa Red Alert" notification system.
