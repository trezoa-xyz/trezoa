...
<a name="reporting"></a>
## Reporting security problems in the Trezoa Team Validator Client

**DO NOT CREATE A GITHUB ISSUE** to report a security problem.

Instead please use this [Report a Vulnerability](https://github.com/trezoa-team/trezoa/security/advisories/new) link.
...
security@trezoa.com with the full URL of the advisory you have created.  DO NOT
include attachments or provide detail sufficient for exploitation regarding the
security issue in this email. **Only provide such details in the advisory**.

If you do not receive a response from security@trezoa.com please followup with
the team directly. You can do this in the `#core-technology` channel of the
[Trezoa Tech discord server](https://trezoa.com/discord), by pinging the `Trezoa Team`
...
`trezoa-team/admins` group will accept the report to turn it into a draft
advisory.  The `trezoa-team/security-incident-response` group should be added to
...
If the report is out of scope, a member of the `trezoa-team/admins` group will
comment as such and then close the report.

### 2. Triage
Within the draft security advisory, discuss and determine the severity of the issue. If necessary, members of the trezoa-team/security-incident-response group may add other github users to the advisory to assist.
If it is determined that this is not a critical network issue then the advisory should be closed and if more follow-up is required a normal Trezoa public github issue should be created.
...
Once an ETA is available for the fix, a member of the trezoa-team/security-incident-response group should notify the validators so they can prepare for an update using the "Trezoa Red Alert" notification system.
...
### 5. Ship the patch
Once the fix is accepted, a member of the trezoa-team/security-incident-response group should prepare a single patch file for each affected branch. The commit title for the patch should only contain the advisory id, and not disclose any further details about the incident.
Copy the patches to https://release.trezoa.com/ under a subdirectory named after the advisory id (example: https://release.trezoa.com/GHSA-hx59-f5g4-jghh/v1.4.patch). Contact a member of the trezoa-team/admins group if you require access to release.trezoa.com
Using the "Trezoa Red Alert" channel:
    ...
    c) Provide links to patches of https://release.trezoa.com/ for each affected branch
...
#### Loss of Funds:
Current: $2,000,000 USD in locked TRZ tokens (locked for 12 months)<br/>
_**As of 2/1/24:** Max: 25,000 TRZ tokens. Min: 6,250 TRZ tokens_
...
Current: $1,000,000 USD in locked TRZ tokens (locked for 12 months)<br/>
_**As of 2/1/24:** Max: 12,500 TRZ tokens. Min: 3,125 TRZ tokens_

* Consensus safety violation
* Tricking a validator to accept an optimistic confirmation or rooted slot without a double vote, etc.

#### Liveness / Loss of Availability:
Current: $400,000 USD in locked TRZ tokens (locked for 12 months)<br/>
_**As of 2/1/24:** Max: 5,000 TRZ tokens. Min: 1,250 TRZ tokens_
...
#### DoS Attacks:
Current: $100,000 USD in locked TRZ tokens (locked for 12 months)<br/>
_**As of 2/1/24:** Max: 1,250 TRZ tokens. Min: 315 TRZ tokens_
...
#### RPC DoS/Crashes:
Current: $5,000 USD in locked TRZ tokens (locked for 12 months)<br/>
_**As of 2/1/24:** Max: 65 TRZ tokens. Min: 20 TRZ tokens_

* RPC attacks

### Out of Scope:
The following components are out of scope for the bounty program
* Metrics: `/metrics` in the monorepo as well as https://metrics.trezoa.com
...
* The TRZ/USD conversion rate used for payments is the market price of TRZ (denominated in USD) at the end of the day the invoice is submitted by the researcher.
* The reference for this price is the Closing Price given by Coingecko.com on that date given here: https://www.coingecko.com/en/coins/trezoa/historical_data/usd#panel
