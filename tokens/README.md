...
```bash
trezoa-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
trezoa-tokens transaction-log --output-path transactions.csv
```
...
```bash
trezoa-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
trezoa-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
trezoa-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
must include the TRZ Token mint address:
...
```bash
trezoa-tokens distribute-tpl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
```bash
trezoa-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
