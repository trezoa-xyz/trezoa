...
local|tar|skip)
  ...
  trezoa_cli=trezoa
  trezoa_gossip=trezoa-gossip
  trezoa_install=trezoa-install
  ;;
*)
...
(
  set -x
  $trezoa_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $trezoa_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: trezoa-install test"
