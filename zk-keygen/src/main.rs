...
use {
    ...
    trezoa_sdk::signer::{EncodableKey, SeedDerivable},
    ...
    match subcommand {
        ("new", matches) => {
            ...
            let outfile = if matches.is_present("outfile") {
                ...
                path.extend([".config", "trezoa", key_type.default_file_name()]);
...
mod tests {
    ...
    fn process_test_command(args: &[&str]) -> Result<(), Box<dyn error::Error>> {
        ...
        let app_matches = app(trezoa_version).get_matches_from(args);
        do_main(&app_matches)
    }
    ...
    fn test_arguments() {
        ...
        app(trezoa_version).debug_assert();
    }
    ...
    fn test_pubkey() {
        ...
        process_test_command(&["trezoa-keygen", "pubkey", "elgamal", &keypair_path]).unwrap();
    }
}
