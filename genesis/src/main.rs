...
use {
    ...
    trezoa_genesis::{genesis_accounts::add_genesis_accounts, Base64Account},
    ...
    trezoa_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    trezoa_stake_program::add_genesis_accounts(&mut genesis_config);
...
mod tests {
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            trezoa_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: trezoa_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
