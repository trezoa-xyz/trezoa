use {
    ...
    trezoa_program_test::{ProgramTest, ProgramTestContext},
...
async fn test_vended_core_bpf_programs() {
    ...
    assert_bpf_program(&context, &trezoa_sdk_ids::address_lookup_table::id()).await;
    assert_bpf_program(&context, &trezoa_sdk_ids::config::id()).await;
    assert_bpf_program(&context, &trezoa_sdk_ids::feature::id()).await;
    assert_bpf_program(&context, &trezoa_sdk_ids::stake::id()).await;
}

// Note this test has to use an entry from `trezoa_builtins::BUILTINS` with a
