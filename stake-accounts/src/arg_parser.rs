...
fn fee_payer_arg<'a, 'b>() -> Arg<'a, 'b> {
    trezoa_clap_utils::fee_payer::fee_payer_arg().required(true)
}
    ...
    App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("url")
                ...
                .takes_value(true)
                .value_name("URL")
                .help("RPC entrypoint address. i.e. http://api.devnet.trezoa.com"),
        )
        .subcommand(
...
fn parse_new_args(matches: &ArgMatches<'_>) -> NewArgs<String, String> {
    NewArgs {
        ...
        lamports: trz_to_lamports(value_t_or_exit!(matches, "amount", f64)),
