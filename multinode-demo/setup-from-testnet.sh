...
rm -rf "$TREZOA_CONFIG_DIR"/latest-testnet-snapshot
mkdir -p "$TREZOA_CONFIG_DIR"/latest-testnet-snapshot
(
  cd "$TREZOA_CONFIG_DIR"/latest-testnet-snapshot || exit 1
  set -x
  wget http://api.testnet.trezoa.com/genesis.tar.bz2
  wget --trust-server-names http://testnet.trezoa.com/snapshot.tar.bz2
)

snapshot=$(ls "$TREZOA_CONFIG_DIR"/latest-testnet-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$trezoa_ledger_tool create-snapshot \
  --ledger "$TREZOA_CONFIG_DIR"/latest-testnet-snapshot \
