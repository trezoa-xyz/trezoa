...
echo 'Building the trezoa cli from source...'
...
cargo build -p trezoa-cli

usage=$(cargo -q run -p trezoa-cli -- -C ~/.foo --help | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')
...
while read -r subcommand rest; do
  ...
  if ((in_subcommands)); then
      section "$(cargo -q run -p trezoa-cli -- help "$subcommand" | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')" "###" >> "$out"
  fi
done <<<"$usage">>"$out"
