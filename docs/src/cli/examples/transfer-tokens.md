---
title: Send and Receive Tokens with the Trezoa CLI
pagination_label: "Trezoa CLI: Send and Receive Tokens"
sidebar_label: Send and Receive Tokens
---

This page describes how to receive and send TRZ tokens using the command line
...
```bash
trezoa airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.trezoa.com
```
...
```bash
trezoa confirm -v <TRANSACTION_SIGNATURE>
```

#### Check your balance

Confirm the airdrop was successful by checking the account's balance.
It should output `1 TRZ`:

```bash
trezoa balance <ACCOUNT_ADDRESS> --url https://api.devnet.trezoa.com
```
...
```bash
trezoa-keygen new --no-passphrase --no-outfile
```
...
The Trezoa cluster will only accept the transfer if you sign the transaction
with the private keypair corresponding to the sender's public key in the
transaction.

```bash
trezoa transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.trezoa.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `trezoa balance`:

```bash
trezoa balance <ACCOUNT_ADDRESS> --url http://api.devnet.trezoa.com
```
...
```bash
$ trezoa-keygen new --outfile my_trezoa_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_trezoa_wallet.json
==========================================================================
...
$ trezoa airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.trezoa.com  # Airdropping 1 TRZ to my wallet's address/pubkey
Requesting airdrop of 1 TRZ from 35.233.193.70:9900
1 TRZ

$ trezoa balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.trezoa.com # Check the address's balance
1 TRZ

$ trezoa-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ trezoa transfer --from my_trezoa_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.trezoa.com --fee-payer my_trezoa_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ trezoa balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.trezoa.com
0.499995 TRZ  # The sending account has slightly less than 0.5 TRZ remaining due to the 0.000005 TRZ transaction fee payment

$ trezoa balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.trezoa.com
0.5 TRZ  # The second wallet has now received the 0.5 TRZ transfer from the first wallet
...
Trezoa, the wallet address is the public key of a keypair. There are a variety
...
If you already hold TRZ and want to send tokens to someone, you will need
...
with the `trezoa transfer` command:

```bash
trezoa transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `trezoa balance`:

```bash
trezoa balance <ACCOUNT_ADDRESS>
```
