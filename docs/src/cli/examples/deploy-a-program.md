...
[programs](https://trezoa.com/docs/terminology#program) (often called smart
contracts elsewhere) with the Trezoa tools.

To learn about developing and executing programs on Trezoa, start with the
[intro to Trezoa programs](https://trezoa.com/docs/core/programs) and then dig
into the details of
[developing on-chain programs](https://trezoa.com/docs/programs).

To deploy a program, use the Trezoa tools to interact with the on-chain loader
to:
...
```bash
trezoa program deploy <PROGRAM_FILEPATH>
```
...
```bash
trezoa program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
trezoa program show <ACCOUNT_ADDRESS>
```
...
```bash
trezoa program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
```bash
trezoa program extend <PROGRAM_ID> <ADDITIONAL_BYTES>
```
...
`trezoa-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [BUFFER_SIGNER] to `trezoa program deploy` or `trezoa program write-buffer'.
Or to recover the account's lamports, pass it as the
[BUFFER_ACCOUNT_ADDRESS] argument to `trezoa program drain`.
==================================================================================
```
...
```bash
trezoa-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
trezoa program deploy --buffer <KEYPAIR_PATH> <PROGRAM_FILEPATH>
```
...
```bash
trezoa program show --programs
trezoa program show --buffers
```

To specify a different authority:

```bash
trezoa program show --programs --buffer-authority <AUTHORITY_ADDRESS>
trezoa program show --buffers --buffer-authority <AUTHORITY_ADDRESS>
```

To close a single account:

```bash
trezoa program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
trezoa program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
trezoa program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```

To close all the buffer accounts associated with the current authority:

```bash
trezoa program close --buffers
```

To show all buffer accounts regardless of the authority

```bash
trezoa program show --buffers --all
```
...
```bash
trezoa program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
trezoa program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
trezoa program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
trezoa program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
trezoa program set-upgrade-authority <PROGRAM_ADDRESS> --final
```
...
```bash
trezoa program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ trezoa dump <ACCOUNT_ADDRESS> dump.so
...
```bash
trezoa program write-buffer <PROGRAM_FILEPATH>
```
...
```bash
trezoa program write-buffer <PROGRAM_FILEPATH> --buffer-authority <BUFFER_AUTHORITY_SIGNER>
```
...
`trezoa program set-buffer-authority`:

```bash
trezoa program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
```bash
trezoa program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
...
trezoa program set-buffer-authority <BUFFER_PUBKEY> --new-buffer-authority <OFFLINE_SIGNER_PUBKEY>
```
...
trezoa program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --sign-only --fee-payer <ONLINE_SIGNER_PUBKEY> --upgrade-authority <OFFLINE_SIGNER> --blockhash <VALUE>

# (5) (use online machine) use this signature to build and broadcast the upgrade transaction on-chain
trezoa program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --fee-payer <ONLINE_SIGNER> --upgrade-authority <OFFLINE_SIGNER_PUBKEY> --blockhash <VALUE> --signer <OFFLINE_SIGNER_PUBKEY>:<OFFLINE_SIGNER_SIGNATURE>
```
Note:
