...
Make sure you have
[installed the Trezoa Command Line Tools](../install.md)

## Generate a File System Wallet Keypair

Use Trezoa's command-line tool `trezoa-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-trezoa-wallet
trezoa-keygen new --outfile ~/my-trezoa-wallet/my-keypair.json
```
...
`~/my-trezoa-wallet/my-keypair.json`. The public key of the keypair file is
...
```bash
trezoa-keygen verify <PUBKEY> ~/my-trezoa-wallet/my-keypair.json
```
