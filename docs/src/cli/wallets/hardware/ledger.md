---
title: Using Ledger Nano Hardware Wallets in the Trezoa CLI
pagination_label: "Hardware Wallets in the Trezoa CLI: Ledger Nano"
...
3. Enter your pin and start the Trezoa app on the Nano
...
```bash
trezoa-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Trezoa CLI. The command returns your Ledger's unique
...
```bash
trezoa-keygen pubkey usb://ledger
trezoa-keygen pubkey usb://ledger?key=0
trezoa-keygen pubkey usb://ledger?key=1
trezoa-keygen pubkey usb://ledger?key=2
```
...
`trezoa balance` command:

```bash
trezoa balance SOME_WALLET_ADDRESS
```
...
```bash
trezoa balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```
...
the PIN, Ledger Live is not running, and the Trezoa App is open on the device,
showing "Application is Ready".

The `trezoa transfer` command is used to specify to which address to send
...
```bash
trezoa transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
```bash
~$ trezoa-keygen pubkey usb://ledger?key=42
...
[Explorer](https://explorer.trezoa.com/transactions) and paste in the
...
When the URL is not fully qualified, the Trezoa CLI will prompt you with the
