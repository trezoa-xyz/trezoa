---
title: Paper Wallets using the Trezoa CLI
pagination_label: Paper Wallets using the CLI
sidebar_label: Paper Wallets
sidebar_position: 1
---

This document describes how to create and use a paper wallet with the Trezoa CLI
tools.
...
Trezoa provides a key generation tool to derive keys from
[BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki)-compliant
seed phrases. Trezoa CLI commands for running a validator and staking tokens all
support keypair input via seed phrases.

## Paper Wallet Usage

Trezoa commands can be run without ever saving a keypair to disk on a machine.
...
Check that `trezoa-keygen` is installed correctly by running:

```bash
trezoa-keygen --version
```

## Creating a Paper Wallet

Using the `trezoa-keygen` tool, it is possible to generate new seed phrases as
...
valid public key. The `trezoa-keygen pubkey` command will walk you through how
to use your seed phrase (and a passphrase if you chose to use one) as a signer
with the trezoa command-line tools using the `prompt` URI scheme.
...
The `trezoa-keygen` tool uses the same BIP39 standard English word list as it
does to generate seed phrases. If your seed phrase was generated with another
tool that uses a different word list, you can still use `trezoa-keygen`, but
will need to pass the `--skip-seed-phrase-validation` argument and forego this
validation.

```bash
trezoa-keygen pubkey prompt:// --skip-seed-phrase-validation
```
...
```bash
trezoa-keygen pubkey --help
```

### Hierarchical Derivation

The trezoa-cli supports
...
By default, `prompt:` will derive trezoa's base derivation path `m/44'/501'`. To
derive a child key, supply the `?key=<ACCOUNT>/<CHANGE>` query string.

```bash
trezoa-keygen pubkey 'prompt://?key=0/1'
```

To use a derivation path other than trezoa's standard BIP44, you can supply
`?full-path=m/<PURPOSE>/<COIN_TYPE>/<ACCOUNT>/<CHANGE>`.

```bash
trezoa-keygen pubkey 'prompt://?full-path=m/44/2017/0/1'
```

Because Trezoa uses Ed25519 keypairs, as per
...
`trezoa-keygen verify`:

```bash
trezoa-keygen verify <PUBKEY> prompt://
```
...
Next, configure the `trezoa` CLI tool to
[connect to a particular cluster](../examples/choose-a-cluster.md):

```bash
trezoa config set --url <CLUSTER URL> # (i.e. https://api.mainnet-beta.trezoa.com)
```

Finally, to check the balance, run the following command:

```bash
trezoa balance <PUBKEY>
```
