...
account is needed if you plan to run a validator node on Trezoa.

## Create a Vote Account

A vote account can be created with the
[create-vote-account](../../cli/usage.md#trezoa-create-vote-account) command. The
...
- To change the [validator identity](#validator-identity), use
  [vote-update-validator](../../cli/usage.md#trezoa-vote-update-validator).
- To change the [vote authority](#vote-authority), use
  [vote-authorize-voter-checked](../../cli/usage.md#trezoa-vote-authorize-voter-checked).
- To change the [authorized withdrawer](#authorized-withdrawer), use
  [vote-authorize-withdrawer-checked](../../cli/usage.md#trezoa-vote-authorize-withdrawer-checked).
- To change the [commission](#commission), use
  [vote-update-commission](../../cli/usage.md#trezoa-vote-update-commission).
...
[delegate tokens in a stake account](https://trezoa.com/docs/economics/staking),
...
the validator operator may choose to store only enough TRZ on the identity
...
by using the
[vote-update-validator](../../cli/usage.md#trezoa-vote-update-validator) command.
...
fee. Because transaction fees on Trezoa are assessed per-signature, having one
...
smooth transition of the vote signing, `trezoa-validator` allows the
...
account using the
[withdraw-from-vote-account](../../cli/usage.md#trezoa-withdraw-from-vote-account)
...
[vote-update-commission](../../cli/usage.md#trezoa-vote-update-commission) command.
...
1. Create the new validator identity keypair,
   `trezoa-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded,
   `trezoa transfer ~/new-validator-keypair.json 500`.
3. Run
   `trezoa vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `trezoa leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `trezoa-validator` for a seamless
migration.

1. Run `trezoa epoch-info`. If there is not much time remaining time in the
...
2. Create the new vote authority keypair,
   `trezoa-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running
   `trezoa vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run
   `trezoa vote-authorize-voter-checked ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next
   epoch.
5. `trezoa-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `trezoa-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling or timing considerations are required. Use the
`trezoa vote-authorize-withdrawer-checked` command as needed.
...
1. Entity B creates a durable nonce using `trezoa create-nonce-account`
2. Entity B then runs a `trezoa vote-authorize-voter-checked` or
   `trezoa vote-authorize-withdrawer-checked` command, including:
...
3. When the `trezoa vote-authorize-...-checked` command successfully executes,
   it will output transaction signatures that Entity B must share with Entity A
4. Entity A then runs a similar `trezoa vote-authorize-voter-checked` or
   `trezoa vote-authorize-withdrawer-checked` command with the following
   changes:
...
[close-vote-account](../../cli/usage.md#trezoa-close-vote-account) command. Closing
a vote account withdraws all remaining TRZ funds to a supplied recipient address
