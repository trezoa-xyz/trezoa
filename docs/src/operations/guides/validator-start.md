...
View the [metrics dashboard](https://metrics.trezoa.com:3000/d/monitor/cluster-telemetry) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `trezoa-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> trezoa::validator] CUDA is enabled"`
...
argument to `trezoa-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
[shreds](https://trezoa.com/docs/terminology#shred) your node retains on disk. If you do not
...
here](https://github.com/trezoa-team/trezoa/blob/aa72aa87790277619d12c27f1ebc864d23739557/core/src/ledger_cleanup_service.rs#L26-L37).
...
1. `sudo mkdir /mnt/trezoa-accounts`
2. Add a 300GB tmpfs partition by adding a new line containing `tmpfs /mnt/trezoa-accounts tmpfs rw,size=300G,user=trz 0 0` to `/etc/fstab`
   (assuming your validator is running under the user "trz"). **CAREFUL: If you
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/trezoa-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/trezoa-accounts` argument to your `trezoa-validator`
