...
### Primary Validator
The following additional `trezoa-validator` parameters are required to enable
...
following `trezoa-validator` command-line argument changes:
...
```bash
$ trezoa-validator wait-for-restart-window --identity validator-keypair.json \
  && trezoa-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `trezoa-validator set-identity
