...
## Run trezoa validator-info

Run the trezoa CLI to populate a validator info account:

```bash
trezoa validator-info publish --keypair ~/validator-keypair.json <VALIDATOR_INFO_ARGS> <VALIDATOR_NAME>
```

For details about optional fields for VALIDATOR_INFO_ARGS:

```bash
trezoa validator-info publish --help
```
...
```bash
trezoa validator-info publish "Elvis Validator" -w "https://elvis-validates.com" -i "https://elvis-validates.com/my-icon.png"
```

Example query command:

```bash
trezoa validator-info get
```
