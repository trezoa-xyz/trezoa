...
```bash
trezoa catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your Trezoa
runtime directory.

```bash
trezoa-keygen new -o ~/validator-stake-keypair.json
```
...
```bash
trezoa delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```

## Validator Stake Warm-up

To combat various attacks on consensus, new stake delegations are subject to a
[warm-up](https://trezoa.com/docs/references/staking/stake-accounts#delegation-warmup-and-cooldown) period.
...
- Look for log messages on your validator indicating your next leader slot:
  `[2019-09-27T20:16:00.319721164Z INFO trezoa_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your
  validator by running `trezoa validators`
...
[RPC API “getRecentBlockhash”](https://trezoa.com/docs/rpc/deprecated/getrecentblockhash) before submitting
a transaction.
...
[leader](https://trezoa.com/docs/terminology#leader)

- After your validator is caught up, use the `trezoa balance` command to monitor
  the earnings as your validator is selected as leader and collects transaction
  fees
- Trezoa nodes offer a number of useful JSON-RPC methods to return information
...
- `getEpochInfo`[An epoch](https://trezoa.com/docs/terminology#epoch) is the
  time, i.e. number of [slots](https://trezoa.com/docs/terminology#slot), for
  which a [leader schedule](https://trezoa.com/docs/terminology#leader-schedule)
...
- `getVoteAccounts` This will tell you how much active stake your validator
  ...
  You can learn more about staking on Trezoa
...
```bash
trezoa deactivate-stake ~/validator-stake-keypair.json
```
