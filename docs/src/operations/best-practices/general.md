---
title: Trezoa Validator Operations Best Practices
...
It is a best practice to always build your Trezoa binaries from source. If you
...
your `trezoa-validator` binary to your specific hardware.
...
`trezoa-install` command.

Assuming you want to install Trezoa version `1.14.17`, you would execute the
following:

```
trezoa-install init 1.14.17
```

This command downloads the executable for `1.14.17` and installs it into a
`.local` directory. You can also look at `trezoa-install --help` for more
options.

> **Note** this command only works if you already have the trezoa cli installed.
> If you do not have the cli installed, refer to
> [install trezoa cli tools](../../cli/install.md)
...
address of the validator in using the `trezoa gossip` command. In the example
below, `5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` is the pubkey of one of my
known validators:

```
trezoa gossip | grep 5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on
```
...
running the `trezoa-validator` software. How much TRZ you should store there is
