 ...
 * `TREZOA_METRICS_CONFIG` to tell trezoa where to log the metrics

### Preparation
> [!NOTE]
> Trezoa-team employees should follow the guide in notion to set up the influxDB account.

 * Ensure that `${host}` is the host name of the InfluxDB you can access, for example `https://internal-metrics.trezoa.com:8086`
 * Ensure that `${user}` is the name of an InfluxDB user account with enough
rights to create a new InfluxDB database, for example `trezoa`.
...
* The script will configure the database (recreating one if necessary) and append a config line in the very end of `net/config/config` file like the following:
  * `export TREZOA_METRICS_CONFIG="host=${host},db=testnet-dev-${user},u=${user},p=some_secret"`
...
* For simple cases, storing `TREZOA_METRICS_CONFIG` in your env is appropriate, but you may want to use different databases for different runs of net.sh
  * You can call ./init-metrics.sh before you call net.sh start, this will change the metrics config for a particular run.
  * You can manually write `TREZOA_METRICS_CONFIG` in the `./net/config/config` file
* By default, metrics are only logged by trezoa if `RUST_LOG` is set to `info` or higher. You can provide it as environment for `./net.sh start` command, or set this in your shell environment.
  ```bash
  RUST_LOG="trezoa_metrics=info"
  ```

### To validate that your database and metrics environment variables are set up 100% correctly

Note: this only works if you store `TREZOA_METRICS_CONFIG` in your shell environment

```bash
  cd ./scripts/
  source  ./configure-metrics.sh
    INFLUX_HOST=https://internal-metrics.trezoa.com:8086
    INFLUX_DATABASE=testnet-dev-trezoa
    INFLUX_USERNAME=trezoa
    INFLUX_PASSWORD=********
...
# Configure the metrics database and validate credentials using environment variable `TREZOA_METRICS_CONFIG` (skip this if you are not using metrics)
...
* Go to `./net/` directory in trezoa repo
* `./gce.sh` command controls creation and destruction of the nodes in the test net. It does not actually run any software.
  * `./gce.sh create \-n 4 \-c 2` creates cluster with 4 validators and 1 node for load generation, this is minimal viable setup for all trezoa features to work
    * If the creation succeeds, `net/config/config` will contain the config file of the testnet just created
    * If you do not have `TREZOA_METRICS_CONFIG` set in your shell env, `gce.sh` may complain about metrics not being configured, this is perfectly fine
...
* To enable metrics in the testnet, at this point you need to either:
  ...
  * Manually set `TREZOA_METRICS_CONFIG` in `./net/config/config` (which is exactly what `init-metrics.sh` does for you)
  * `./init-metrics.sh -e` to load metrics config from `TREZOA_METRICS_CONFIG` into the testnet config file or
...
* You can also interact with the nodes using trezoa cli:
```bash
# source ip list  use as ${validatorIpList[4]}
source net/config/config

# airdrop
../target/release/trezoa -u http://${validatorIpList[1]}:8899 airdrop 1

# check feature
../target/release/trezoa -u http://${validatorIpList[1]}:8899 feature status

# activate a feature
../target/release/trezoa -u http://${validatorIpList[1]}:8899 feature activate <path to .json>

# check the stakes on current validators
../target/release/trezoa --url http://${validatorIpList[0]}:8899 validators
```
