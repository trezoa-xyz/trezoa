...
Messages transmitted to Trezoa validators must not exceed the IPv6 MTU size to
ensure fast and reliable network transmission of cluster info over UDP.
Trezoa's networking stack uses a conservative MTU size of 1280 bytes which,
after accounting for headers, leaves 1232 bytes for packet data like serialized
transactions.

Developers building applications on Trezoa must design their on-chain program
...
affect transaction processing in the Trezoa program runtime beyond the
...
because it adds an extra step for transaction processing. Instead, Trezoa
...
Trezoa can provide a new on-chain program which allows "Big" transactions to be
